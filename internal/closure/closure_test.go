...
func TestEvaluateSkipsClosureBasisWhenNotRequired(t *testing.T) {
	root := t.TempDir()
	repo := filepath.Join(root, "repo")
	stateDir := filepath.Join(root, "state")
	initClosureGitRepo(t, repo)
	...
	if _, err := policy.Bind(policy.BindOptions{StateDir: stateDir, ConfigPath: writeClosurePolicyWithBasis(t, root, []string{"fixed"}), Profile: "relaxed"}); err != nil {
		t.Fatalf("Bind relaxed policy: %v", err)
	}
	if _, err := snapshot.Capture(snapshot.CaptureOptions{StateDir: stateDir, RepoPath: repo, Kind: "base", Ref: "HEAD"}); err != nil {
		t.Fatalf("Capture base: %v", err)
	}
	...
	relaxed, err := closure.Evaluate(closure.EvaluateOptions{StateDir: stateDir, PolicyProfile: "relaxed"})
	if err != nil {
		t.Fatalf("Evaluate relaxed closure: %v", err)
	}
	if !relaxed.Closed || hasClosureBlocker(relaxed.Blockers, "unsatisfied_closure_basis") {
		t.Fatalf("require_basis_for_unresolved=false should not enforce allowed basis: %+v", relaxed)
	}
}
...
func TestEvaluateDoesNotCountContextExpansionAsPrimaryLimit(t *testing.T) {
	root := t.TempDir()
	repo := filepath.Join(root, "repo")
	stateDir := filepath.Join(root, "state")
	initClosureGitRepo(t, repo)
	...
	if _, err := reviewresult.Import(reviewresult.ImportOptions{
		...
		ResultPath: writeClosureWorkerResult(t, reviewresult.WorkerResult{
			...
			Route:         reviewresult.RoutePrimaryReview,
			Outcome:       reviewresult.OutcomeClean,
			Summary:       "No actionable findings after the allowed context expansion.",
		}),
	}); err != nil {
		t.Fatalf("Import clean result: %v", err)
	}
...
func writeClosurePolicyWithBasis(t *testing.T, root string, allowed []string) string {
  ...
  "profiles": {
    ...
    "relaxed": {
      ...
      "required_evidence_facts": ["primary_review_completed", "blocking_findings_verified", "coverage_obligations_satisfied", "policy_bound"],
      "risk_routing": [],
      "closure_basis": {"allowed_basis": ` + string(allowedBody) + `, "require_basis_for_unresolved": false}
    }
  }
}`)
