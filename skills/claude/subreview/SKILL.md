...
For code review, capture and review the proposal before final capture when possible: base snapshot, proposal snapshot, base-to-proposal diff, `obligations build`, primary packet, external proposal review, `result import`. After final capture, create base-to-final and proposal-to-final diffs, rebuild obligations, record final gate evidence, build the final primary packet, run/import the external final review, then build targeted verification packets only for proposal-origin findings. In a final manifest, `packet build --kind primary` targets `base->final`; it cannot recreate a proposal primary packet for missing proposal evidence.

Use `subreview result validate --json` when you need to check worker-result JSON before import. It uses the same strict result rules as import but writes no CAS objects and appends no ledger events. For verification packets, repeat `--finding` to target multiple proposal findings in one packet; result JSON must provide exactly one logical verdict per packet finding. Final-target findings are not accepted by targeted verification. Use `--max-context-bytes <n>` when the default packet context budget is too small, and treat bounded context omissions or `needs_context` results as a reason to rebuild/import rather than as closure.
...
Do not claim code-review closure from a clean reviewer response alone. Code-review closure must come from `subreview close`, which evaluates the latest policy-bound ledger evidence and reports facts, blockers, next_actions, gates, findings, discovery/verification runs, token telemetry, and scheduler status. A final primary review satisfies `independent_final_completed`, but it does not satisfy stronger policy facts such as `fresh_blinded_review` or `cli_witnessed`; when those facts are required, follow the no-command `next_actions` guidance instead of inventing unsupported commands.
